...
void traversarePreordine(NodArb* rad)
{
	if (rad != nullptr)
	{
		cout << "Cod= " << rad->inf.cod << " nume= " << rad->inf.nume << " varsta= " 
			<< rad->inf.varsta << " BF = "<<rad->BF<< endl;
...
void drumDeLaRadLaUnNod(NodArb* rad, int cod, int* i, bool* gasit)
{
	if (rad != nullptr)
	{
		if (rad->inf.cod == cod)
		{
			cout << "Gasit dupa " << *i << " cautari" << endl;
			cout << "Cod= " << rad->inf.cod << " nume= " <<
				rad->inf.nume << " varsta= " <<
				rad->inf.varsta << " BF = " << rad->BF << endl;
			*gasit = true;
			return;
		...
		if (!(*gasit))
		{
			...
			if (cod > rad->inf.cod)
			{
				drumDeLaRadLaUnNod(rad->dreapta, cod, i, gasit);
			}
			else
			{
				drumDeLaRadLaUnNod(rad->stanga, cod, i, gasit);
			}
		}
	}
}
