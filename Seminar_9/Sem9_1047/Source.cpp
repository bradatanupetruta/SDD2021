...
void traversarePreordine(NodArb* rad)
{
	if (rad != nullptr)
	{
		cout << "PREORD=> Cod= " << rad->inf.cod << ", Editura: " << rad->inf.editura << ", Pret: " << rad->inf.pret << ", An publicatie: " << rad->inf.an_publicare <<" , BF = "<<rad->BF<< endl;
...
void traversareInordine(NodArb* rad)
{
	if (rad != nullptr)
	{
		...
		cout << "INORD=> Cod= " << rad->inf.cod << ", Editura: " << rad->inf.editura << ", Pret: " << rad->inf.pret << ", An publicatie: " << rad->inf.an_publicare << " , BF = " << rad->BF << endl;
...
void traversarePostordine(NodArb* rad)
{
	if (rad != nullptr)
	{
		...
		cout << "POSTORD=> Cod= " << rad->inf.cod << ", Editura: " << rad->inf.editura << ", Pret: " << rad->inf.pret << ", An publicatie: " << rad->inf.an_publicare << " , BF = " << rad->BF << endl;
	}
}
...
//afisare noduri pe un anumit nivel
void noduriPeNivel(NodArb* rad, int n, int inaltime)
{
	if (rad != nullptr)
	{
		if (nrNiveluri(rad) == inaltime - n + 1) {
			cout << "Cod= " << rad->inf.cod << ", Editura: " << rad->inf.editura
				<< ", Pret: " << rad->inf.pret << ", An publicatie: "
				<< rad->inf.an_publicare << " BF=" << rad->BF << endl;
		}
		noduriPeNivel(rad->st, n - 1, inaltime - 1);
		noduriPeNivel(rad->dr, n - 1, inaltime - 1);
	}
}

//traversare arbore pe niveluri
void afisarePeNiveluri(NodArb* rad) {
	int h = nrNiveluri(rad);
	for (int i = 0; i < h; i++) {
		cout << "Nivel " << i + 1 << endl << endl;
		noduriPeNivel(rad, i + 1, h);
		cout << endl;
	}
}
